...
{
        if (argc < 4) {
                std::cerr << "usage: " << std::string(basename(argv[0])) << " MESSAGE PROTO JSON" << std::endl;
                return EXIT_FAILURE;
        }
	...
	int main(void) {
	    std::ifstream fle(")" + json + R"(");
	    if (!fle.is_open()) {
		    std::cerr << "could not open file: " << ")" + json + R"(" << std::endl;
		    return EXIT_FAILURE;
	    }
	    ...
	    json2pb(msg, buffer.str().c_str(), buffer.str().size());
